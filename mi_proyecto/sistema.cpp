...
void Sistema::vCercano(float px, float py, float pz, const std::string& nombreObjeto) {
  // Obtener todos los objetos del manejador
  const auto &objetos = manejadorObj.obtenerTodosLosObjetos();

  // Verificar si el objeto existe
  Objeto3D* objeto = nullptr;
  for (const auto &obj : objetos) {
      if (obj->getNombreMalla() == nombreObjeto) {
          objeto = obj;
          break;
      }
  }
  ...
  ArbolKD* arbol = objeto->getArbolKD();
  ...
  if (verticeCercano != nullptr) {
      ...
      std::cout << "Resultado exitoso: El vértice (" << verticeCercano->getX()
                << ", " << verticeCercano->getY() << ", "
                << verticeCercano->getZ() << ") del objeto " << nombreObjeto
                << " es el más cercano al punto (" << px << ", " << py << ", "
                << pz << "), a una distancia de " << distancia << "." << std::endl;
  }
}
...
void Sistema::vCercanoGlobal(float px, float py, float pz) {
  if (manejadorObj.estaVacio())
    ...
    {
        std::cout << "No hay vertices cargados en ningun objeto." << std::endl;
        return;
    }
